        ...
        try {
            for (id in cameraManager!!.cameraIdList) {
                ...
                if (hasFlash == true && facing == CameraCharacteristics.LENS_FACING_BACK) {
                    ...
                    Log.i("FlashlightManager", "Max strength: " + characteristics.get(CameraCharacteristics.FLASH_INFO_STRENGTH_MAXIMUM_LEVEL))
                    maxStrength = characteristics.get(CameraCharacteristics.FLASH_INFO_STRENGTH_MAXIMUM_LEVEL) ?: 50
                    return turnOn(maxStrength / 2)
                }
            }
...
fun FlashlightDialog(onFinish: () -> Unit, darkTheme: Boolean = isSystemInDarkTheme()) {
    ...
    var lightLevel by remember { mutableFloatStateOf(FlashlightManager.getMaxStrength().toFloat() / 2) }
                    ...
                    if (supportsBrightness) {
                        Slider(
                            ...
                            onValueChange = { newValue ->
                                ...
                                val clickEffect = VibrationEffect.createOneShot(50, (lightLevel.toInt() * 2))
                                vibrator.vibrate(clickEffect)

                                FlashlightManager.turnOn(lightLevel.toInt().coerceAtLeast(1))

                            },
                            valueRange = 1f..maxStrength,
